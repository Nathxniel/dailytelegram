...
function createSimblaObj(form) {
    ...
    function save() {
        ...
        function done(refresh) {
            T.data('disabled', false)
            if (cls == QueryString.cls && QueryString.status && QueryString.status.toLowerCase() == "new") {
    ...
    try {
        ...
        if (o.data('criteria')) {
            ...
            criteria.forEach(function (f) {
                ...
                if (val !== undefined && val !== "" && (typeof val != "number" || !isNaN(val)) && (!(f.P && f.P.targetClass) || val != null))
    ...
    function buildList(results) {
        function replaceIsoDate(html, field, date) {
            if (html.indexOf("{{" + field + ".format(") > -1) {
                var reg = new RegExp("{{" + field + ".format\\(([a-zA-Z,0-1-]+)\\)}}", "g");
                var foundArr = [];
                var groupArr;
...
function simblaTable() {
    ...
    function setTdHtml(value, type, o) {
        switch (type) {
            ...
            case 'Number':
                if (value && value % 1 != 0)
                    return o.text(Number(value).toFixed(2));
                o.text(value);
                break;
            case 'String':
    ...
    function exportToCsv() {
        ...
        return addCriteriaToQuery(query, cls, m).find().then(function (results, err) {
    ...
    function summaryLine() {
        if (!((pages && page === pages) || num <= getLimit())) return m.trigger('summary-loaded', {
            data: {}
        });
...
$(function () {
    ...
    $('body').on('click', '.logout a', function () {
        ...
        Simbla.User.logOut().then(function () {
            ...
            if ($('meta[name="reseller-source"]').attr('logout-url'))
...
function getSelectPointers(targetClass, selectfields, value, T, name) {
    ...
    function setResult(d) {
        ...
        d.forEach(function (o) {
            ...
            T.append($('<option>').attr('value', o.id).text(decodeEntities(name ? (name == "objectId" ? o.id : o.get(name)) : getTextKeyValue(o, T))));
        });
        if (val != undefined) {
            if (T.prop('multiple')) {
                val = val.split(',')
